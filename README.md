...
**Live**: [jobfit-rag.vercel.app](https://jobfit-rag.vercel.app) (frontend, Vercel) — backed by
a FastAPI service on Railway.
...
- **Backend** (Railway): the FastAPI service, Postgres, and a persistent volume mounted at
  `/app/chroma_data` (Chroma's index has to survive restarts/redeploys). Build/start config
  lives in `railpack.json` at the repo root — the start command runs `alembic upgrade head`
  before starting `uvicorn`, so migrations apply automatically on every deploy.
- **Frontend** (Vercel): stateless, just needs `BACKEND_URL` set to the Railway service's public
  domain as a production environment variable. No persistent storage needed since all data
  lives behind the backend.

Both platforms were chosen specifically because the in-process APScheduler job-sync loop needs
a long-running process (not a serverless function) and Chroma needs real persistent disk —
most "free" PaaS tiers offer neither.
